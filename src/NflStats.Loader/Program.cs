...
namespace NflStats.Loader
{
    class Program
    {
                ...
                Parallel.ForEach(files, new ParallelOptions { MaxDegreeOfParallelism = 5 }, (file) =>
                {
                    ...
                    if (!string.IsNullOrEmpty(json))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                var original = repo.GetScheduleItems(homeScheduleItem.TeamId).Where(c => c.GameId == homeScheduleItem.GameId).FirstOrDefault();
                                if (original != null)
                                {
                            ...
                            if (repo.Exists(awayScheduleItem))
                            {
                                var original = repo.GetScheduleItems(awayScheduleItem.TeamId).Where(c => c.GameId == awayScheduleItem.GameId).FirstOrDefault();
                                if (original != null)
                                {
                            ...
                            if (gameModel.Matchup.Boxscore.Count > 0)
                            {
                        ...
                        {
                            logger.LogError(ex, $"ERROR PROCESSING GAME ID {gameModel.GameId}");                            
                        }
        ...
        static void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient<IProcessor<ReceivingModel>>(provider => new ReceivingProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<PassingModel>>(provider => new PassingProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<RushingModel>>(provider => new RushingProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<FumbleModel>>(provider => new FumbleProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<DefenseModel>>(provider => new DefenseProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<InterceptionModel>>(provider => new InterceptionProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<ReturnModel>>(provider => new ReturnProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<KickingModel>>(provider => new KickingProcessor(provider.GetService<SqlContext>()));
            services.AddTransient<IProcessor<PuntingModel>>(provider => new PuntingProcessor(provider.GetService<SqlContext>()));
            services.AddTransient(provider => new ScoringProcessor(provider.GetService<SqlContext>()));
            services.AddTransient(provider => new TeamStatProcessor(provider.GetService<SqlContext>()));
